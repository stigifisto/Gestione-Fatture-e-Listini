...
Public Class frmPrezziFattureElettroniche
    ...
    ''' cerca i fornitori con fatture nel periodo dtpDal/dtpAl e apre frmScegliFornitore per
    ...
    ''' <summary>
    ''' Elenca, in ordine alfabetico di descrizione, i cedenti/fornitori (Fatture_Testate) con
    ''' almeno una fattura nel range di data indicato, con il relativo conteggio fatture.
    ''' </summary>
    Private Function GetFornitoriConFattureNelPeriodo(dal As Date, al As Date) As DataTable
        Dim sql As String = "
            SELECT
                t.CedenteIdCodice AS Codice,
                t.CedenteDenominazione AS Descrizione,
                COUNT(DISTINCT t.ID_Fattura) AS NumeroFatture
            FROM Fatture_Testate t
            WHERE t.DataFattura BETWEEN @dal AND @al
            GROUP BY t.CedenteIdCodice, t.CedenteDenominazione
            ORDER BY t.CedenteDenominazione"
    ...
    Private Sub AggiornaFiltro()
        ...
        If testo.Length > 0 Then
            ...
            condizioni.Add($"NumeroFattura LIKE '%{valoreEscaped}%'")
        End If
